    ...
    return (
      <div style={{ padding: '40px 16px', textAlign: 'center' }}>
        ...
        <div style={{ fontSize: 13, color: 'var(--muted)', lineHeight: 1.6 }}>
          ...
          <span style={{ color: 'var(--accent)', fontWeight: 700 }}>Save to Library</span>
          {' '}to store it here.
        </div>
      </div>
    )
  ...
  return (
    <div style={{ paddingBottom: 16 }}>
      <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 10 }}>
        <button type="button" onClick={onClearAll} className="btn btn-ghost btn-sm-mobile"
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <MatBlock label="Water"    accent="var(--accent)" fieldVal={waterField} ssdVal={waterSSD} unit="kg/m³" />
        <MatBlock label="Sand (FA)" accent="var(--warn)"  fieldVal={sandField}  ssdVal={sandSSD}  unit="kg/m³" />
      </div>

      {warns.length > 0 && (
        <div style={{ margin: '0 14px 10px' }}>
          {warns.map((w, i) => (
            <div key={i} className="alert alert-warn" style={{ marginBottom: 4 }}>{w.msg}</div>
          ))}
        </div>
      )}

      <div style={{ display: 'flex', gap: 8, padding: '0 14px 14px' }}>
        <button type="button" onClick={onLoad} className="btn btn-primary" style={{ flex: 1 }}>
          Load Inputs
        </button>
        <button type="button" onClick={onDelete} className="btn btn-danger" style={{ padding: '0 18px' }}>
          Delete
        </button>
      </div>
    </div>
